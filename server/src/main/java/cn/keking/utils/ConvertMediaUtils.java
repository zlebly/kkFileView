...
public class ConvertMediaUtils {
    ...
    private static final List<Integer> MP4_CODEC_LIST =
            Arrays.asList(avcodec.AV_CODEC_ID_H264, avcodec.AV_CODEC_ID_MPEG4, avcodec.AV_CODEC_ID_H263);

    private static final List<Integer> FLV_CODEC_LIST = Arrays.asList(avcodec.AV_CODEC_ID_H264, avcodec.AV_CODEC_ID_H263);
    public static Boolean checkAvcodec(FileAttribute fileAttribute) {
        ...
        try {
            ...
            if ("mp4".equalsIgnoreCase(fileAttribute.getSuffix())
                    && !MP4_CODEC_LIST.contains(frameGrabber.getVideoCodec())) {
                return true;
            }
